...
const getTimezoneOffset = (date: Date): string => {
  ...
  return `${sign}${hours}`; // Devolver el número con signo
};
...
export const formatTime = (
  ...
  switch (format) {
    ...
    case "GMT_LOCAL": {
      const targetDate = localDate; // Para GMT_LOCAL, usamos la fecha local
      ...
      const timezoneOffset = getTimezoneOffset(targetDate); // Obtener el desfase horario
      return `${monthName} ${day}, ${year} ${timeString} GMT${timezoneOffset}`; // Con GMT y el offset
    }
    default:
      return date.toLocaleString(language, formatOptions);
  }
